...
class Model{
	...
	public function insert($tab , array $data){
		foreach($data as $k=>$v){		
		 ...
		 $final_column = implode("," , $column);
		 $final_symbol = implode("," , $symbol);		 
	$sql = "insert into ".$tab. "(".$final_column.") values (".$final_symbol.")";
...
public function deletes($tab,array $where,$con="and"){
		//for base
		foreach($where as $base_k=>$base_v){
			$store_base_data[]=$base_k."='".$base_v."'";
		}
	...
	public function get_wishlist_product($cus_id){
		$sql="select id,title,sale_price,status from products where id in(select product_id from wishlist where customer_id=".$cus_id.")";
		$stmt=$this->db->prepare($sql);
		$stmt->execute();
		if($stmt->rowCount()>0){
			while($row=$stmt->fetch(PDO::FETCH_OBJ)){
 ...
 $sql="select * from ".$tab." LIMIT $start,$per_page";
  ...
  $stmt->execute(); 
	...
	* count row of cart
	*/
	public function countCart(int $cid){
		$sql="select id from cart where c_id=".$cid;
		$stmt=$this->db->prepare($sql);
		$stmt->execute();
	...
	/////////
		//get wishlist product
	public function get_order_product($cus_id){
		$sql="select id,title,sale_price,status from products where id in(select p_id from cart where c_id=".$cus_id.")";
